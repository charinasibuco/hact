	...
	<div class='large-12 columns'>
		...
		<table width="100%">
			...
			<tr class="{{ ($row->is_mortality == 0) ? '' : 'mortality' }}">
				<td>
					<a href="{{ route('laboratory_show', $row->id) }}" title="View Laboratories"><i class="fa fa-folder-open fa-lg"></i></a>
					<a href="{{ route('laboratory_create', $row->id) }}" title="Create New laboratory"><i class="fa fa-plus fa-lg"></i></a>
				</td>
				<td>{{ $row->code_name }}</td>
				<td>{{ $row->age }}</td>
