...
<div class="row">
	<div class="large-7 columns">
		<ul class="breadcrumbs">
			...
			<li class="current"><a href="#">TUBERCULOSIS</a></li>
		</ul>
	</div>
	...
	<div class='large-12 columns'>
		...
		<table width="100%">
			...
			<tr class="{{ ($row->is_mortality == 0) ? '' : 'mortality' }}">
				<td>
					<a href="{{ route('tuberculosis_show', $row->id) }}" title="TB History"><i class="fa fa-folder-open fa-lg"></i></a>
					<a href="{{ route('tuberculosis_create', $row->id) }}" title="New TB Information"><i class="fa fa-plus fa-lg"></i></a>
				</td>
				<td>{{ $row->code_name }}</td>
				<td>{{ $row->age }}</td>
			...
			@endforeach
			<!-- @include('hact.placeholders.with_record') -->
			</tbody>
		</table>
		{!! str_replace('/?', '?', $patients->appends($pagination)->render()) !!}
	</div>
